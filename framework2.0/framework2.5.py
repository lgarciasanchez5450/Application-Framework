...
from math import sqrt, cos, sin, hypot,atan2,pi,acos
...
if sys.platform == 'win32':
  ...
  try:
    default_font_name = font.get_fonts()[0]
  except IndexError:
...
class Input:
  ...
  def get_all(self,list_of_things) -> tuple:
...
class Options: #In Development
  def __init__(self,pos):
    self.options = dict()
    self.pos = pos
    self.font = font.SysFont(default_font_name,20)
...
class DesmosSlider:
  ...
  def __init__(self,pos,size,lower_limit,upper_limit,save_function,slider_color,ball_color,starting_value,scale,fontsize,abs_lower=None,abs_upper=None):
    assert lower_limit <= upper_limit, "lower limit must be less than upper limit"
    assert abs_lower <= abs_upper, "absolute lower limit must be less than absolute upper limit"
    self.lower_limit = lower_limit
    self.upper_limit = upper_limit
    self.abs_lower = abs_lower
    self.abs_upper = abs_upper
    end_box_sizex = scale * fontsize * 16/21 # ratio of width to height of font for numbers
    slider_end_box_space = 3
    ...
    self.min_box = InputBox(pos,(end_box_sizex,size[1]),'Lower',slider_color,scale,self.set_lower_limit,numbers,fontsize)
    self.max_box = InputBox((pos[0]+size[0]-end_box_sizex,pos[1]),(end_box_sizex,size[1]),'Upper',slider_color,scale,self.set_upper_limit,numbers,size[1])
  ...
  def set_lower_limit(self,new_lower_limit):
    ...
    if self.abs_lower is not None and new_lower_limit < self.abs_lower: #if lower than absolute lower
      new_lower_limit = self.abs_lower
      self.min_box.set_text(str(new_lower_limit))
    if new_lower_limit > self.upper_limit:
      new_lower_limit = self.upper_limit
      self.min_box.set_text(str(new_lower_limit))
  ...
  def set_upper_limit(self,new_upper_limit):
    ...
    new_upper_limit += 1
    if self.abs_upper is not None and new_upper_limit > self.abs_upper: #if higher than the absolute highest
      new_upper_limit = self.abs_upper
      self.min_box.set_text(str(new_upper_limit))
    if new_upper_limit < self.lower_limit:
      new_upper_limit = self.lower_limit
      self.max_box.set_text(str(new_upper_limit))
    self.upper_limit = new_upper_limit
    self.slider.changeSliderLimits(None,new_upper_limit)
...
class Dropdown:
  ...
  def __init__(self,pos,size,up_color,down_color,text_color,captions,outPutCommand,maxy,rightClickCommand = None,tpos = (0,0),spacing:int = 1, myfont = None ):
    ...
    self.font = font.SysFont(default_font_name, 20) if myfont is None else myfont
    self.yscroll = 0
    self.mhover = 0
    self.pmhover = 0
    self.maxy = maxy
...
class InputBox:
  ...
  def __init__(self,pos,size,caption = '',box_color = (100,100,100),max_chars=500,save_function = lambda x:x,restrict_input = None,fontSize = 21):
    self.pos = pos
    self.size = size
    self.font = font.SysFont(default_font_name,fontSize)
